...
Public Class frmCekmesizSevkiyat
...
#Region " Windows Form Designer generated code "
    ...
    Friend WithEvents Musteri As System.Windows.Forms.DataGridTextBoxColumn

    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frmCekmesizSevkiyat))
        ...
        Me.Musteri = New System.Windows.Forms.DataGridTextBoxColumn
        ...
        '
        Me.Musteri.Format = ""
        Me.Musteri.FormatInfo = Nothing
        Me.Musteri.HeaderText = "Musteri"
        Me.Musteri.MappingName = "Musteri"
        Me.Musteri.NullText = ""
        Me.Musteri.Width = 100
        'Panel1
        '
        ...
        'txtPaletNo
        '
        Me.txtPaletNo.Location = New System.Drawing.Point(64, 8)
        ...
        Me.btnSend.Size = New System.Drawing.Size(84, 21)
        ...
        Me.btnCreatePalet.Size = New System.Drawing.Size(84, 21)
        ...
        Me.Panel3.Size = New System.Drawing.Size(240, 227)
        '
        'dtgrdMain
        '
        ...
        Me.Name = "frmCekmesizSevkiyat"
        Me.Text = "Çekmesiz Sevkiyat"
...
#Region " Methods "
    ...
    Private Sub bindData()
        ...
        column = New DataGridTextBoxColumn
        column.MappingName = "Musteri"
        column.HeaderText = "Musteri"
        column.Width = 100
        tablestyle.GridColumnStyles.Add(column)
        ...
        Try
            ...
            (ds.Tables(0).Rows.Count >= 1) Then
                ...
                For Each dr2 As DataRow In ds.Tables("Sevk").Rows
                    ...
                    Sorgu = "SELECT SUM(ADAQQT-ISNULL(Uf_Okutulan,0)) ADAQQT FROM CekmeListesiMst " & _
                    " WHERE ADA3CD = '" & frmKullaniciGirisi.ambar & "'" & _
                    " AND C6CANB + CAST(cust_seq AS nvarchar(5)) = " & dr2("Musteri") & _
                    " AND ADAITX = '" & dr2("Malzeme") & "'" & _
                    " AND qty_packed = 0 "
                    dt2 = srv.RunSqlDs(Sorgu, "CekmeListesiMst", Mobile.ProviderTypes.SqlClient).Tables(0)

                    cekMik = dt2.Rows(0)("ADAQQT")

                    If cekMik < sevkMik Then
                        'MessageBox.Show(dr2("Malzeme") & " nolu malzemeden en fazla " & cekMik & " adet sevk edebilirsiniz.")
                        If message = "" Then
                            message = dr2("Malzeme") & " nolu malzemeden en fazla " & cekMik & " adet sevk edebilirsiniz."
                        Else
                            message = message & _
                            vbNewLine & _
                            dr2("Malzeme") & " nolu malzemeden en fazla " & cekMik & " adet sevk edebilirsiniz."
                        End If
                ...
                For Each row As DataRow In ds.Tables("Sevk").Rows
                    ...
                    Sorgu = "SELECT (ADAQQT - ISNULL(Uf_Okutulan,0)) Kalan , * FROM CekmeListesiMst " & _
                    " WHERE ADA3CD = '" & frmKullaniciGirisi.ambar & "'" & _
                    " AND C6CANB + CAST( cust_seq AS nvarchar(5)) = '" & row("Musteri") & "'" & _
                    " AND ADAITX = '" & row("Malzeme").ToString & "'" & _
                    " AND (ADAQQT - ISNULL(Uf_Okutulan,0)) > 0 " & _
                    " AND qty_packed = 0 " & _
                    " ORDER BY ADBJDT"
                    dt2 = srv.RunSqlDs(Sorgu, "CekmeListesiMst", Mobile.ProviderTypes.SqlClient).Tables(0)
                        ...
                        If satirToplam <> 0 Then
                            If toplam <> 0 And toplam > 0 Then
                                If miktar > kalan Then
                                    Sorgu = "UPDATE coitem  " & _
                                    " SET Uf_Okutulan = ISNULL(Uf_Okutulan,0) + " & kalan & "" & _
                                    " WHERE co_num ='" & row2("ADCVNB") & "' AND co_line =" & row2("ADFCNB") & _
                                    " AND item = '" & row2("ADAITX") & "'"
                                ...
                                Else
                                    Sorgu = "UPDATE coitem  " & _
                                    " SET Uf_Okutulan = ISNULL(Uf_Okutulan,0) + " & miktar & "" & _
                                    " WHERE co_num ='" & row2("ADCVNB") & "' AND co_line =" & row2("ADFCNB") & _
                                    " AND item = '" & row2("ADAITX") & "'"
                                    srv.RunSql(Sorgu, True, Mobile.ProviderTypes.SqlClient)
...
#End Region
    ...
    Private Sub frmCekmesizSevkiyat_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            Cursor.Current = Cursors.WaitCursor
    ...
    Private Sub frmCekmesizSevkiyat_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles MyBase.Closing
        If Formclosing() Then
            e.Cancel = False
        Else
            e.Cancel = True
    ...
    Private Sub btnDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnDelete.Click
        ...
        dr = MessageBox.Show("Seçili paleti yada koli etiketini silmek istediðinizden emin misiniz?", "Onay", _
        ...
        If dr = Windows.Forms.DialogResult.Yes Then
            ...
            If Not (ds Is Nothing) And (ds.Tables(0).Rows.Count >= 1) Then
                ...
                If dt2.Rows.Count > 0 Then
                    ...
                    If dt2.Rows(0)("cekmeno") <> 1 Then
                        MessageBox.Show("Bu palet sevk edilmemiþ.")
                        Clear()
                        Exit Sub
                    End If
                ...
                Else
                    ...
                    If dt2.Rows(0)("cekmeno") <> 1 Then
                        MessageBox.Show("Bu koli etiketi sevk edilmemiþ.")
                        Clear()
                        Exit Sub
                    End If
                ...
                For Each row As DataRow In ds.Tables("Sevk").Rows
                    ...
                    Sorgu = "SELECT Uf_Okutulan , * FROM CekmeListesiMst " & _
                    " WHERE C6CANB + cast( cust_seq as nvarchar(5)) = '" & row("Musteri") & "'" & _
                    " AND ADAITX ='" & row("Malzeme").ToString & "'" & _
                    " AND ISNULL(Uf_Okutulan,0) > 0 " & _
                    " AND qty_packed = 0 " & _
                    " ORDER BY ADBJDT desc"
                    dt2 = srv.RunSqlDs(Sorgu, "CekmeListesiMst", Mobile.ProviderTypes.SqlClient).Tables(0)

                    For Each row2 As DataRow In dt2.Rows

                        Dim okutulan As Integer = 0
                        okutulan = row2("Uf_Okutulan")
                        satirToplam = row("adet").ToString
                        ...
                        If satirToplam <> 0 Then
                            If toplam <> 0 And toplam > 0 Then
                                If miktar > okutulan Then
                                    Sorgu = "Update coitem  " & _
                                    " Set Uf_Okutulan = Uf_Okutulan - " & okutulan & "" & _
                                    " Where co_num ='" & row2("ADCVNB") & "' AND co_line =" & row2("ADFCNB")
                                    srv.RunSql(Sorgu, True, Mobile.ProviderTypes.SqlClient)
                                ...
                                Else
                                    Sorgu = "Update coitem  " & _
                                    " Set Uf_Okutulan = Uf_Okutulan - " & miktar & "" & _
                                    " Where co_num ='" & row2("ADCVNB") & "' AND co_line =" & row2("ADFCNB")
                                    srv.RunSql(Sorgu, True, Mobile.ProviderTypes.SqlClient)
        ...
        Try
            ...
            If e.KeyCode = Keys.Enter Then
                ...
                If dt.Rows.Count > 0 Then
                    ...
                    Sorgu = "SELECT firmakoduzun + cast( cust_seq as nvarchar(5)) Musteri,* FROM Veri_Toplama.dbo.[TR_PALET] " & _
                    " WHERE palet_no = '" & txtPaletNo.Text & "'"
                ...
                ElseIf dt2.Rows.Count > 0 Then
                    ...
                    Sorgu = "SELECT firmakoduzun + cast( cust_seq as nvarchar(5)) Musteri,* FROM Veri_Toplama.dbo.[SYTELINEPALETLEME] " & _
                    " WHERE sirano = '" & txtPaletNo.Text & "'"
                    dt2 = srv.RunSqlDs(Sorgu, "SYTELINEPALETLEME", Mobile.ProviderTypes.SqlClient).Tables(0)
